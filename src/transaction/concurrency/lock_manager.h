...
public:
    ...
    void set_transaction_manager(TransactionManager *txn_mgr) {
      transaction_manager_ = txn_mgr;
    }

    // 获取事务管理器指针（用于MVCC支持）
    TransactionManager *get_transaction_manager() { return transaction_manager_; }
...
private:
    ...
    TransactionManager *transaction_manager_ = nullptr; // 事务管理器指针，用于MVCC支持
};