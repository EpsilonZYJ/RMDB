...
class IxIndexHandle {
   ...
   public:
    ...
    bool clear_pages() {
        // Delete all pages associated with this index file
        for (page_id_t page_id = 0; page_id < file_hdr_->num_pages_; page_id++) {
            PageId pid = {fd_, page_id};
            buffer_pool_manager_->unpin_page(pid, false);
            buffer_pool_manager_->delete_page(pid);
        }
        return true;
    }
    bool has_key(const char *key, Transaction *transaction, bool unique_check=true);
   private:
    // 辅助函数
    void update_root_page_no(page_id_t root) { file_hdr_->root_page_ = root; }
