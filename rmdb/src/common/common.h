...
struct Condition {
    ...
    bool check(const RmRecord& record, TabMeta& tab_) {
        // 获取列信息

        // 获取左操作数的值
        Value lhs_value;
        int lhs_offset = tab_.get_col(lhs_col.col_name)->offset;
        lhs_value.type = tab_.get_col(lhs_col.col_name)->type;
        int len = tab_.get_col(lhs_col.col_name)->len;
        ...
        if (is_rhs_val) {
            ...
            int rhs_offset = tab_.get_col(rhs_col.col_name)->offset;
            rhs_value.type = tab_.get_col(rhs_col.col_name)->type;
            int len = tab_.get_col(rhs_col.col_name)->len;
