...
class AntiJoinColumnError : public RMDBError {
    public:
        explicit AntiJoinColumnError(const std::string &column_names) : 
            RMDBError("ANTI JOIN can only select columns from the left table. Invalid columns: " + column_names) {}
    };
